...
$stmt = $con->prepare('SELECT username,nickname,email,password FROM staff WHERE id = ?');
// In this case we can use the account ID to get the account info.
$stmt->bind_param('i', $id);
$stmt->execute();
$stmt->bind_result($username,$nickname,$email,$password);
$stmt->fetch();
$stmt->close();
?>
<!DOCTYPE html>
		...
		<body class="loggedin">
         ...
         <div class="navbar">
           ...
           <div class="navbar">
            ...
            <div class="function">
      <a style='float:left' href='stafflist.php?#<?php echo $username ?>'><i class='fas fa-arrow-circle-left'></i> Back</a>
    </div>
      <div class="function">
		...
		<div class="content">
			<h2>Profile Page</h2>
      <form action="editstaff.php" method="post" autocomplete="off">
          <div class="container">
            <div>
              <label for="username"><b>Staff name</b></label>
                <div>
                  <input type="text" name="nickname" maxlength="25" placeholder="Nickname" id="nickname" value="<?php echo $nickname;?>"required>
                </div>
            </div>
          <div>
