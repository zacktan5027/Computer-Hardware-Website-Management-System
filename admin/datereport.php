...
<html>
	...
	<body class="loggedin">
      ...
      <div class="function">
          <a style="float:right" href="datereport.php?type=table&year=<?php echo $_SESSION['date']?>"><i class="fas fa-table"></i> Summary</a>
          <a style="float:right" href="datereport.php?type=graph&year=<?php echo $_SESSION['date']?>"><i class="fas fa-chart-bar"></i> Graph</a>
          <br /><br />
      </div>

		<div class="content">
      <form method="post" action="datereport.php?type=<?php echo $_GET['type']?>">
        <h2>Input date:</h2>
       <input id="myDate"style="display:block;width:100%;padding: 12px 20px" name="date" required="" type="date" value="" >
         <input onclick="myFunction()"style="width:100%" type="submit" value="Date Report"/>
         <script>
         function myFunction() {
           var today = new Date();
           var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
           var x = document.getElementById("myDate").max = date;
           document.getElementById("demo").innerHTML = "The value of the max attribute was changed from '2000-01-01' to '2014-01-01'.";
         }
       </script>
     </form><br />
     <style>
     ...
     .hoverable tbody tr:hover,.ul.hoverable li:hover{
      ...
      if ($_GET['type']=="table") {
      $sql ="SELECT *,SUM(quantity) as purchasetime FROM purchase natural join purchased where date(timepurchase)= '$date' GROUP BY productid";
      $result = mysqli_query($conn, $sql);
      if (mysqli_num_rows($result) > 0) {
        ?>
        <button class="none" style="background-color: black;color: white;float: right;width:100px" onclick="printFunction()">Print</button><br />
        <script>
        ...
        <?php
          ...
          while ($row = mysqli_fetch_array($result)) {
            ...
            echo "<td style='width:100px'>" . $row['purchasetime'] . "</td>";
