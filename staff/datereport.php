...
<html>
	...
	<body class="loggedin">
      ...
      <form method="post" action="datereport.php?type=<?php echo $_GET['type']?>">
        <h2>Input date:</h2>
       <input style="display:block;width:100%;padding: 12px 20px" name="date" required="" type="date" value="" >
       <input style="width:100%" type="submit" value="Date Report"/>
     </form><br />
      <?php
      ...
      if ($_GET['type']=="table") {
      $sql ="SELECT *,SUM(quantity) as purchasetime FROM purchase natural join purchased GROUP BY productid";
      ...
      while ($row = mysqli_fetch_assoc($result)) {
          if (date('Y-m-d', strtotime($row['timepurchase']))==$date) {
              echo "<tr>";
